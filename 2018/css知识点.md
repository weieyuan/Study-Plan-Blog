...
3.边框会忽略百分比的宽度设置  
4.margin存在"margin collapsing"，当两个box元素相邻时，它们之间的距离是这两个box中margin值最大的那个距离，而不是两者相加。(同号取大，异号相减)。
...
当某个元素发生浮动时，并且该元素后面的元素没有文本内容时，浮动元素会覆盖掉紧随其后的元素；如果该元素后面的元素具有文本内容时，浮动元素会覆盖掉紧随其后的元素，但是文字环绕在浮动元素周围显示。
